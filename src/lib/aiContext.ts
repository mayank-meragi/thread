...
- Recipes are ordinary threads with recipe-specific Markdown roles: #[cook-section] is a non-executable container, nested #[cook-step] blocks are the cooking instructions, and #[cook-note] blocks are supporting context. Indentation determines ownership. In recipe steps, @ingredient{quantity%unit}(preparation) marks ingredients, ^cookware{} or bare ^pan marks cookware, and ~{quantity%unit} or ~label{quantity%unit} marks timers. Cookware uses ^, never #, because #[...] is Thread's structural tag syntax. Recipe details derive ingredients, cookware, timers, sections, notes, serving scaling, and the shopping list from this Markdown; do not invent a separate recipe database or syntax.
