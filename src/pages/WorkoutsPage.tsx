...
function NextWorkout({ workouts }: { workouts: readonly WorkoutView[] }) {
  ...
  return (
    <section className={`workout-next state-${state}`}>
      ...
      <ButtonLink to={`/workout/${workout.task.day}/${workout.task.id}/overview`}>{state === 'active' ? 'Resume workout' : 'View workout'}</ButtonLink>
    </section>
  )
}
...
function ExerciseIndexRow({ exercise, selected, onSelect }: { exercise: ExerciseProgress; selected: boolean; onSelect: () => void }) {
  ...
  return (
    <ToggleButton unstyled pressed={selected} type="button" className={`exercise-index-row${selected ? ' selected' : ''}`} onClick={onSelect}>
...
export function WorkoutsPage() {
  ...
  return (
    <article className="workouts-page">
      ...
      {workouts.length === 0 ? (
        <div className="workouts-empty"><Dumbbell size={24} aria-hidden="true" /><h2>Build your training history</h2><p>Create your first workout. Completed sessions will turn into trends, records, and exercise progress here.</p><Button disabled={busy} onClick={() => void newWorkout()}><Plus size={16} /> New workout</Button></div>
      ) : view === 'overview' ? (
        ...
        <div className="workout-history-view">
          ...
          <section className="workout-log">
            ...
            <div className="workout-log-filters">
              ...
              <div className="workout-status-filter field-with-icon"><ListFilter size={15} aria-hidden="true" /><Select aria-label="Filter by status" value={status} onChange={(event) => setParam('status', event.target.value === 'all' ? undefined : event.target.value)}><option value="all">All statuses</option><option value="active">Active</option><option value="planned">Planned</option><option value="completed">Completed</option><option value="canceled">Canceled</option></Select></div>
            </div>
