...
import { Button, Field, Input, MenuItem } from 'fiber'
...
export function FeedsPage() {
  ...
  const renderFeedRow = (feed: FeedRecord) => (
    <div className="feed-source-row-wrap" key={feed.id}>
      <Button unstyled type="button" className={`feed-source-row${selectedSource.kind === 'feed' && selectedSource.id === feed.id ? ' is-active' : ''}`} onClick={() => selectSource({ kind: 'feed', id: feed.id })} title={feed.lastError ?? feed.title}>
