...
import { ArrowLeft, Check, ChevronDown, ChevronRight, ExternalLink, FileUp, Folder, FolderPlus, MoreHorizontal, Pencil, Plus, RefreshCw, Rss, Trash2, X } from 'lucide-react'
...
export function FeedsPage() {
  ...
  }, [dialog, filter, refreshCurrentSource, selectedEntry, visibleEntries])
  ...
  return (
    <article className="feeds-page" aria-keyshortcuts="J K O M R">
      <header className="feeds-topbar"><div className="feeds-topbar-title"><Rss size={16} /><strong>Feeds</strong><span className="feeds-shortcuts">J/K navigate · O open · M read · R refresh</span></div><div className="feeds-topbar-actions"><button type="button" className="secondary-button" onClick={() => void refreshAll()} disabled={refreshing.size > 0 || feeds.length === 0}><RefreshCw size={15} className={refreshing.size > 0 ? 'feeds-spin' : undefined} /> <span>Refresh</span></button><button type="button" className="secondary-button" onClick={openImport}><FileUp size={15} /> <span>Import</span></button><button type="button" className="primary-button" onClick={openSubscribe}><Plus size={15} /> <span>Add feed</span></button></div></header>

      <div className="feeds-workspace" data-mobile-pane={mobilePane}>
        <aside className="feeds-sources" aria-label="Subscriptions"><div className="feeds-panel-head"><span>Subscriptions</span><div><span className="feeds-panel-count">{feeds.length}</span><button type="button" className="feed-panel-action" aria-label="Create folder" onClick={() => openFolderDialog('create')}><FolderPlus size={15} /></button></div></div><div className="feeds-source-scroll"><button type="button" className={`feed-source-row${selectedSource.kind === 'all' ? ' is-active' : ''}`} onClick={() => selectSource({ kind: 'all' })}><span className="feed-source-icon"><Rss size={15} /></span><span className="feed-source-name">All feeds</span>{totalUnread > 0 && <b>{totalUnread}</b>}</button>{folders.map((folder) => { const collapsed = collapsedFolderIds.has(folder.id); return <div className="feed-source-group" key={folder.id}><div className="feed-folder-row-wrap"><button type="button" className={`feed-source-row feed-folder-row${selectedSource.kind === 'folder' && selectedSource.id === folder.id ? ' is-active' : ''}`} onClick={() => selectSource({ kind: 'folder', id: folder.id })}><span className="feed-source-icon"><Folder size={14} /></span><span className="feed-source-name">{folder.name}</span>{(folderUnread.get(folder.id) ?? 0) > 0 && <b>{folderUnread.get(folder.id)}</b>}</button><button type="button" className="feed-folder-toggle" aria-label={`${collapsed ? 'Expand' : 'Collapse'} ${folder.name}`} aria-expanded={!collapsed} onClick={(event) => { event.stopPropagation(); toggleFolder(folder.id) }}>{collapsed ? <ChevronRight size={14} /> : <ChevronDown size={14} />}</button><button type="button" className="feed-source-more" aria-label={`Actions for ${folder.name}`} onClick={() => setSourceMenuId(sourceMenuId === `folder:${folder.id}` ? null : `folder:${folder.id}`)}><MoreHorizontal size={15} /></button>{sourceMenuId === `folder:${folder.id}` && <div className="feed-source-menu menu-panel"><button type="button" className="menu-item" onClick={() => openFolderDialog('rename', folder)}><Pencil size={14} /> Rename</button><button type="button" className="menu-item feed-menu-danger" onClick={() => openFolderDelete(folder)}><Trash2 size={14} /> Delete folder</button></div>}</div>{!collapsed && feeds.filter((feed) => feed.folderId === folder.id).map(renderFeedRow)}</div> })}{feeds.some((feed) => !feed.folderId) && <div className="feed-source-group feed-ungrouped"><div className="feed-group-label">Ungrouped</div>{feeds.filter((feed) => !feed.folderId).map(renderFeedRow)}</div>}{feeds.length === 0 && <div className="feeds-empty-source"><p>No subscriptions yet.</p><button type="button" onClick={openSubscribe}>Add your first feed</button></div>}</div></aside>

        <section className="feeds-inbox" aria-label="Feed inbox"><div className="feeds-inbox-head"><button type="button" className="feed-mobile-back" onClick={() => setMobilePane('sources')}><ArrowLeft size={15} /> Sources</button><div className="feeds-tabs" role="tablist"><button type="button" role="tab" aria-selected={filter === 'all'} className={filter === 'all' ? 'is-active' : ''} onClick={() => setFilter('all')}>All <span>{entries.length}</span></button><button type="button" role="tab" aria-selected={filter === 'unread'} className={filter === 'unread' ? 'is-active' : ''} onClick={() => setFilter('unread')}>Unread <span>{entries.filter((entry) => !entry.readAt).length}</span></button></div><div className="feeds-bulk-actions"><button type="button" className="feeds-mark-all" onClick={() => void markCurrentSource(true)} disabled={!entries.some((entry) => !entry.readAt)}><Check size={14} /> Mark all read</button><button type="button" className="feeds-mark-all" onClick={() => void markCurrentSource(false)} disabled={!entries.some((entry) => entry.readAt)}>Mark all unread</button></div></div><div className="feeds-entry-list">{visibleEntries.map((entry) => { const source = feeds.find((feed) => feed.id === entry.feedId); return <button type="button" data-feed-entry-id={entry.id} className={`feed-entry-row${selectedEntry?.id === entry.id ? ' is-active' : ''}${entry.readAt ? '' : ' is-unread'}`} onClick={() => void selectEntry(entry)} key={entry.id}><span className="feed-entry-marker" aria-hidden="true" /><span className="feed-entry-copy"><strong>{entry.title}</strong><span>{source?.title ?? 'Feed'} · {relativeDate(entry.publishedAt ?? entry.fetchedAt)}</span></span></button> })}{visibleEntries.length === 0 && <div className="feeds-empty-list"><Rss size={25} /><strong>{feeds.length === 0 ? 'Your reading list starts here.' : filter === 'unread' ? 'You are caught up.' : 'No entries cached yet.'}</strong><p>{feeds.length === 0 ? 'Add a feed to bring a little more signal into your day.' : 'Refresh a subscription to check for new entries.'}</p></div>}</div></section>

        <section className="feed-reader" aria-label="Article detail"><div className="feed-reader-head"><button type="button" className="feed-mobile-back" onClick={() => setMobilePane('entries')}><ArrowLeft size={15} /> Entries</button><span>{selectedFeed?.title ?? selectedFolder?.name ?? (selectedSource.kind === 'all' ? 'All feeds' : 'Feed')}</span>{selectedEntry && <button type="button" className="icon-button" aria-label={selectedEntry.readAt ? 'Mark unread' : 'Mark read'} onClick={() => void markFeedEntryRead(selectedEntry.id, !selectedEntry.readAt)}>{selectedEntry.readAt ? <span className="feed-read-indicator" aria-hidden="true" /> : <Check size={16} />}</button>}</div>{selectedEntry ? <div className="feed-reader-scroll"><div className="feed-reader-body"><h2>{selectedEntry.title}</h2><div className="feed-reader-meta">{selectedEntry.author ? `${selectedEntry.author} · ` : ''}{displayDate(selectedEntry.publishedAt ?? selectedEntry.fetchedAt)}</div>{selectedEntry.summaryHtml ? <div className="feed-reader-content" dangerouslySetInnerHTML={{ __html: sanitizeFeedHtml(selectedEntry.summaryHtml) }} /> : <p className="feed-reader-empty">This entry has no summary. Open the original article to continue reading.</p>}{selectedEntry.url && <a className="feed-original-link" href={selectedEntry.url} target="_blank" rel="noreferrer">Open original article <ExternalLink size={14} /></a>}</div></div> : <div className="feed-reader-empty-state"><Rss size={28} /><strong>Select an entry to read.</strong><p>Your selected feed’s summaries will appear here.</p></div>}</section>
      </div>
