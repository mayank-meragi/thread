  ...
  const renderFeedRow = (feed: FeedRecord) => (
    <div className="feed-source-row-wrap" key={feed.id}>
      <Tooltip content={feed.lastError ?? feed.title}>
        <Button unstyled type="button" className={`feed-source-row${selectedSource.kind === 'feed' && selectedSource.id === feed.id ? ' is-active' : ''}`} onClick={() => selectSource({ kind: 'feed', id: feed.id })}>
